-- =====================================================================
-- 20260916230000_fix_move_capture_workspace_folder_permissions.sql
-- Fix move_capture_to_workspace_folder permissions:
--   1. Allow workspace members with non-viewer roles ('owner', 'creator', 'admin')
--      in addition to workspace owner (workspaces.owner_user_id = auth.uid())
--      for both source and target workspaces.
--   2. Auto-insert target folder into public.workspace_folders on conflict do nothing
--      if a target folder name is provided, ensuring reliable move execution
--      without needing separate folder pre-creation calls.
--   3. Set search_path = public, pg_temp for security linter compliance.
-- =====================================================================
