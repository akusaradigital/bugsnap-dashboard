-- =====================================================================
-- 20260912120000_fix_get_public_capture_expiry.sql
-- Fix get_public_capture to properly check expires_at < now() in addition to burn_after_read,
-- and hide site_url on expired captures.
-- =====================================================================
...
begin
  ...
  if v_rec.password is not null and (p_password is null or p_password <> v_rec.password) then
    return query select
      ...
      v_rec.window_size, v_rec.description, null::jsonb, null::text, null::text,
